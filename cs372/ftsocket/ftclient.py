...
def main():
	...
	dataPort = int(sys.argv[5]) if len(sys.argv) == 6 else int(sys.argv[4])

	if(argc != 5 or argc != 6):
		print "Usage: python ftclient.py <host> <port> <-l | -g> <filename> <dataport>"


	s.close()

	buffer = s.recv(1024) # await the return...
	s.send(dataPort)
