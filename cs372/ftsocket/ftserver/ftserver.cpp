...
int FTPServer::acceptConnection(int sockfd, string &hostBName, unsigned short serverPort)
{
	if(DEBUG)
		cout << "If you see this, the test is working for now in acceptConnection." << endl;

	char hostBNameCh[HOST];
	memset(hostBNameCh, '\0', HOST);
	struct sockaddr_in client;
	socklen_t clientLength = sizeof(client);
	newsockfd = accept(sockfd, (struct sockaddr*) &client, &clientLength);
	getnameinfo((struct sockaddr*)&client, sizeof(client), hostBNameCh, sizeof(hostBNameCh), NULL, 0, 0);
	hostBName = string(hostBNameCh); // force the char to become string
	
	cout << "Connection accepted from " << hostBName << " accepted on " << sockfd << " " << "port: " << serverPort << endl;
	return newsockfd;

}

void FTPServer::listDirectory(int newdatasockfd)
{
	if(DEBUG)
	...
	while (current = readdir(curDir))
	{
		...
		read(newdatasockfd, buffer, BUFFER);
		memset(buffer, '\0', BUFFER);
		strcpy(buffer, current->d_name);
		write(newdatasockfd, buffer, strlen(buffer));
	}
...
void FTPServer::sendFile(int filefd, int newdatasockfd, int fileLength)
{
	...
	write(newdatasockfd, response, strlen(response));
	memset(response, '\0', RESPONSE);
	read(newdatasockfd, response, RESPONSE);
...
void FTPServer::receiveCmd(int newsockfd)
{
	...
	memset(buffer, '\0', BUFFER);
	read(newsockfd, buffer, BUFFER);
	strcpy(serverPortCh, buffer);
	dataPort = atoi(serverPortCh);
	...
	while(1)
	{
		...
		strcpy(buffer, "OK");
		write(newsockfd, buffer, strlen(buffer));
		newdatasockfd = acceptConnection(datasockfd, otherHost, dataPort);
		if(newdatasockfd < 0)
		{
