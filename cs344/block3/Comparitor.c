...
// int executeShell(char **args, int numArgs, int *exitFlag, int *signalFlg, int *termSgnl, bgChild *head);	Potential redundancy
int execute(char **args, int numArgs, int bgProc);
...
int main()
{
	...
	char *userInput, **args;
	int fd, input, output, bgProc, 
		numArgs, shStatus, exitFlag = 0, 
		signalFlg = 0, termSgnl;
